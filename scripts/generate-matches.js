...
fetch('https://api.fifa.com/api/v3/calendar/matches?idCompetition=17&idSeason=285023&language=en&count=200')
  ...
  .then(d => {
    ...
    let output = "const matches = [\n";
    matches.forEach(m => {
      const parts = [`id: ${m.id}`];
      if (m.group) parts.push(`group: '${esc(m.group)}'`);
      parts.push(`stage: '${esc(m.stage)}'`);
      parts.push(`date: '${esc(m.date)}'`);
      parts.push(`time: '${esc(m.time)}'`);
      parts.push(`venue: '${esc(m.venue)}'`);
      parts.push(`homeTeam: '${esc(m.homeTeam)}'`);
      parts.push(`awayTeam: '${esc(m.awayTeam)}'`);
      output += "  { " + parts.join(', ') + " },\n";
    });
    output += "];\n\nexport default matches;\n";
    fs.writeFileSync('src/data/matches.js', output, 'utf8');
    console.log('Written ' + matches.length + ' matches to src/data/matches.js');
  })
